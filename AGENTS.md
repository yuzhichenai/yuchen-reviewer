...
1. 必须先 edit，再做其他事 —— todowrite/bash/Read 都是逃避 edit 的借口
2. 连续 2 次非 edit 工具后，若仍无法切换为 edit，直接向用户报告卡住
3. 反循环规则放在文件最顶部，模型第一眼可见
4. Agent 权限改为 `edit: deny` + `bash: deny`，纯审查模式，不修改代码
